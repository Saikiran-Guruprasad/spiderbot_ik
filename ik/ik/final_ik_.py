...
class HexLeg:
    ...
    def xp(self, t1, x, y, cx, cy):
        leg_dir = self.ang + radians(t1)
        dx, dy  = x - cx, y - cy
        behind  = (dx * cos(leg_dir) + dy * sin(leg_dir)) < 0
    ...
    def theta_1(self, x, y):
        pivot_x = self.l1 * cos(self.ang)
        pivot_y = self.l1 * sin(self.ang)
        angle = atan2(y - pivot_y, x - pivot_x) - self.ang
    ...
    def theta_3(self, x, y, z):
        t1 = self.theta_1(x, y)
        cx, cy = self._coxa(t1)
        xp = self.xp(t1, x, y, cx, cy)
        c = (-xp*xp - z*z + self.l2*self.l2 + self.l3*self.l3) / (2*self.l2*self.l3)
        return (degrees(acos(c)) - 180)
    
    def theta_2(self, x, y, z, t3):
        t1 = self.theta_1(x, y)
        cx, cy = self._coxa(t1)
        xp = self.xp(t1, x, y, cx, cy)
        return (degrees(acos((xp*xp + z*z + self.l2*self.l2 - self.l3*self.l3) / (2*self.l2*sqrt(xp*xp + z*z)))) - degrees(atan2(abs(z), abs(xp))))
...
class Rotation:
    def __init__(self, x, y, z):            #z usage can be tied to delays, later implement
        ...
        self.step_size = (10/z) + 10 if z != 0 else 20 #arbitrary step size chosen, can be changed according to requirements
        self.B = 15 if self.final_angle > 0 else -15 #arbitrary degrees chosen
...
def cal(t, leg, flag, lift=4.0, steps=20):
    P1 = (leg.y0 - 2, leg.z0)
...
class FinalIKNode(Node):
        ...
        if (self.buffer.position_x != self.num.position_x or self.buffer.position_y != self.num.position_y or self.buffer.twist_z != self.num.twist_z):
            if self.is_cycle_complete or (self.num.position_x==0.0 and self.num.position_y==0.0 and self.num.twist_z==0.0) or self.rotation_complete:
                ...
                if self.buffer.position_y==self.num.position_y and self.buffer.position_x==0.0 or self.buffer.position_x==self.num.position_x and self.buffer.position_y==0.0:
                    ...
                    self.output = [[0,25,-75]]*6
                    self.publish_angles(self.output)
                    if self.delayer == 0:
                        self.delay(11)
                        return
                    elif self.delayer == -1:
    ...
    def transverse(self, x_pos, y_pos):
        ...
        self.legs     = [HexLeg(l1=8, lc=2, l2=5, l3=7, ang_deg=a) for a in angles]
        self.output   = [[0, 0, 0]] * 6
        self.t_params = [0.5, 0.0, 0.5, 0.0, 0.5, 0.0]
        self.flags    = [1, -1, 1, -1, 1, -1]
        for i in range(6):
    ...
    def publish_angles(self, output):
        msg = JointState()
        ...
        # Flatten nested list + convert degrees to radians
        msg.position = [
            math.radians(angle)
            for leg in output
            for angle in leg
